...
`define EXPECT_CPLD_PAYLOAD board.xilinx_pci_exp_4_lane_downstream_port.tx_usrapp.expect_cpld_payload
`define EXPECT_MEMWR_PAYLOAD board.xilinx_pci_exp_4_lane_downstream_port.tx_usrapp.expect_memwr_payload
`define EXPECT_MEMWR64_PAYLOAD board.xilinx_pci_exp_4_lane_downstream_port.tx_usrapp.expect_memwr64_payload
  ...
  begin
    ...
    begin
      ...
      $display("[%t] : Received MEMRD64 --- Tag 0x%h", $realtime, tag_);
          $display("[%t] : Found header mismatch in received MEMRD64 - Tag 0x%h: \n", $time, tag_);
          $display("Expected:");
          $display("\t Traffic Class: 0x%h", traffic_class);
          $display("\t TD: %h", td);
          $display("\t EP: %h", ep);
          $display("\t Attributes: 0x%h", attr);
          $display("\t Length: 0x%h", length);
          $display("\t Requester ID: 0x%h", requester_id);
          $display("\t Tag: 0x%h", tag);
          $display("\t Last DW byte-enable: 0x%h", last_dw_be);
          $display("\t First DW byte-enable: 0x%h", first_dw_be);
          $display("\t Address: 0x%h", address);
          $display("Received:");
          $display("\t Traffic Class: 0x%h", traffic_class_);
          $display("\t TD: %h", td_);
          $display("\t EP: %h", ep_);
          $display("\t Attributes: 0x%h", attr_);
          $display("\t Length: 0x%h", length_);
          $display("\t Requester ID: 0x%h", requester_id_);
          $display("\t Tag: 0x%h", tag_);
          $display("\t Last DW byte-enable: 0x%h", last_dw_be_);
          $display("\t First DW byte-enable: 0x%h", first_dw_be_);
          $display("\t Address: 0x%h", address_);
      if(tag == tag_) //find matching tag
      begin
        wait_for_next = 1'b0;
        if((traffic_class == traffic_class_) &&
...
/************************************************************
 * Task : TSK_EXPECT_INTR
 * Inputs : traffic_class, td, ep, attr, length, Message Type,
 *          Message Code, Address High,Address Low
 *          Outputs : status 0-Failed 1-Successful
 *          Description : Expecting a memory write (32-bit address) TLP
 *                        from Rx side with matching header fields and
 *                                      payload
...
task TSK_EXPECT_INTR;
  ...
  reg   [15:0] requester_id_;
  reg   [7:0]  tag_;
  reg   [2:0]  message_type_;
  reg   [7:0]  message_code_;
  ...
  begin
    ...
    while(wait_for_next)
    begin
      @ rcvd_msg; //wait for a rcvd_memwr event
      traffic_class_ = frame_store_rx[1] >> 4;
      td_ = frame_store_rx[2] >> 7;
      ep_ = frame_store_rx[2] >> 6;
      attr_ = frame_store_rx[2] >> 4;
      ...
      requester_id_= {frame_store_rx[4], frame_store_rx[5]};
      tag_= frame_store_rx[6];
      message_type_= frame_store_rx[0];
      message_code_= frame_store_rx[7];

      payload_len = (length << 2);
      if (payload_len==0) payload_len = 4096;

      $display("[%t] : Received Message with no Data --- Tag 0x%h, message_type 0x%h", $realtime, tag_,message_type_ );
      if((tag == tag_) && (message_type == message_type_)) //find matching tag and message_type
      begin
        wait_for_next = 1'b0;
        if((traffic_class == traffic_class_) &&
           (td === td_) && (ep == ep_) && (attr == attr_) &&
           (length == length_) && (requester_id == requester_id_) &&
           (message_type == message_type_) && (message_code == message_code_) )
        begin
                $display("[%t] : Interrupt received as expected. type[0x%h], code[0x%h]", $realtime, message_type_,message_code_); 
            expect_status = 1'b1;
        end
        else // header mismatches, error out
        begin
          ...
          $fdisplay(error_file_ptr, "\t Requester ID: 0x%h", requester_id);
          $fdisplay(error_file_ptr, "\t Tag: 0x%h", tag);
          $fdisplay(error_file_ptr, "\t message_type: 0x%h", message_type);
          $fdisplay(error_file_ptr, "\t message_code: 0x%h", message_code);
          $fdisplay(error_file_ptr, "Received:");
          $fdisplay(error_file_ptr, "\t Traffic Class: 0x%h", traffic_class_);
          $fdisplay(error_file_ptr, "\t TD: %h", td_);
          $fdisplay(error_file_ptr, "\t EP: %h", ep_);
          ...
          $fdisplay(error_file_ptr, "\t Requester ID: 0x%h", requester_id_);
          $fdisplay(error_file_ptr, "\t Tag: 0x%h", tag_);
          $fdisplay(error_file_ptr, "\t message_type: 0x%h", message_type_);
          $fdisplay(error_file_ptr, "\t message_code: 0x%h", message_code_);
          $fdisplay(error_file_ptr, "");
          expect_status = 1'b0;
        end
      end
